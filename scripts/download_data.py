...
DATASET_MAP = {x.__name__.lower(): x for x in DATASETS}
...
def download():
    ...
    parser.add_argument(
        "--n_procs", type=int, default=1, help="Number of processes to download with"
    )
    parser.add_argument(
        "--ignore_errors",
        action="store_true",
    ...
    parser.add_argument(
        "--cache-only",
        action="store_true",
        help="Only download the cached dataset",
    )
    parser.add_argument(
        "--clear-failures",
        action="store_true",
        help="If image previously failed to download, clear it from the cache",
    )
    parser.add_argument(
        "--check-sha",
        action="store_true",
        help="Check the SHA of the dataset",
        default=False,
    )
    ...
    for ix, dataset in enumerate(to_download):
        ...
        logging.info(
            f"Starting download for {dataset.__name__} ({ix + 1}/{len(to_download)})"
        )
        try:
            dataset.download(
                n_procs=args.n_procs,
                retry_failures=args.clear_failures,
                # cache_only=args.cache_only,
                # check_sha=args.check_sha,
            )
            ...
            if args.ignore_errors:
                ...
                raise e
        logging.info(
            f"Done with {dataset.__name__} in {time.perf_counter() - t0:0.1f} seconds"
        )
