...
def convert_state_dict_clip(
    state_dict, vision_config: VisionBackboneConfig
) -> Dict[str, Any]:
    ...
    patch_embedding = (
        state_dict["embeddings"]["patch_embedding"]
        .pop("weight")
        .permute(0, 2, 3, 1)
        .reshape(vision_config.image_emb_dim, -1)
    )
...
def convert_state_dict_dino(
    ...
    patch_embedding = (
        state_dict["embeddings"]["patch_embeddings"]["projection"]
        .pop("weight")
        .permute(0, 2, 3, 1)
        .reshape(vision_config.image_emb_dim, -1)
    )
    patch_embedding_b = state_dict["embeddings"]["patch_embeddings"]["projection"].pop(
        "bias"
    )
...
def convert_state_dict_olmoe(
    state_dict, config: ModelConfig, block_type: BlockType
) -> Dict[str, Any]:
    state_dict = unflatten_dict(state_dict, sep=".")
    ...
    for layer in range(config.n_layers):
        ...
        q, k, v, o = [
            layer_dict["self_attn"][f"{k}_proj"].pop("weight")
            for k in ["q", "k", "v", "o"]
        ]
        ...
        mlp_gates = [
            layer_dict["mlp"]["experts"][str(i)]["gate_proj"].pop("weight")
            for i in range(config.moe_num_experts)
        ]
        mlp_ups = [
            layer_dict["mlp"]["experts"][str(i)]["up_proj"].pop("weight")
            for i in range(config.moe_num_experts)
        ]
        mlp_downs = [
            layer_dict["mlp"]["experts"][str(i)]["down_proj"].pop("weight").t()
            for i in range(config.moe_num_experts)
        ]
        ...
        mapped_layer_dict = {
            ...
            "attn_norm": {"weight": layer_dict[f"input_layernorm"].pop("weight")},
            "att_proj": dict(
...
def convert_state_dict_olmo_1024_preview(
    state_dict, config: ModelConfig, block_type: BlockType
) -> Dict[str, Any]:
    state_dict = unflatten_dict(state_dict, sep=".")
    assert len(state_dict) == 2
...
def convert_state_dict_qwen2(
    ...
    for layer in range(config.n_layers):
        ...
        if config.qkv_bias:
            q_b, k_b, v_b = [
                layer_dict["self_attn"][f"{k}_proj"].pop("bias")
                for k in ["q", "k", "v"]
            ]
        else:
...
def main_vit(args: argparse.Namespace) -> None:
    ...
    logging.info(
        f"Convert model {args.model} to olmo format and save to {output_path}..."
    )
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.data_dir is None:
        if "MOLMO_DATA_DIR" not in os.environ:
            raise ValueError(
                "Either `data_dir` or env variable MOLMO_DATA_DIR must be set"
            )
...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="convert hf vit/llm to molmo format script"
    )
    parser.add_argument(
        "model",
        type=str,
