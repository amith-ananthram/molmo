...
from olmo.lora_utils import wrap_model_with_lora, print_lora_parameters
...
def main(cfg: TrainConfig) -> None:
    ...
    if not cfg.data.multi_modal and cfg.save_dataloader_state:
        raise OLMoConfigurationError(
            "You are saving the dataloader state, but the data is not multi-modal."
        )
            ...
            if config_path.exists():
                ...
                if lastest_checkpoint.exists():
                    ...
                    if saved_config.model != cfg.model:
                        logging.warning(
                            "Model config does not match the one resuming from"
                        )
                    if saved_config.optimizer != cfg.optimizer:
                        logging.warning(
                            "Optimizer config does not match the one resuming from"
                        )
                    if saved_config.data != cfg.data:
                        logging.warning(
                            "Data config does not match the one resuming from"
                        )
        ...
        if get_local_rank() == 0:
            ...
            if cfg.initial_model_checkpoint:
                state_dict = torch.load(
                    join(cfg.initial_model_checkpoint, "model.pt"), map_location="cpu"
                )
    ...
    if olmo_model.config.block_type == "moe":
        log.info(
            f"Number of active parameters: {olmo_model.num_params(include_inactive_params=False):,d}"
        )
    ...
    with Trainer(
        ...
        if cfg.load_path is not None:
            ...
            trainer.restore_checkpoint(
                ...
                load_dataloader_state=cfg.save_dataloader_state
                and not cfg.reset_dataloader_state,
        ...
        if cfg.compile is not None:
            ...
            trainer.train_batch = torch.compile(
                trainer.train_batch, **cfg.compile.asdict()
            )  # type: ignore
